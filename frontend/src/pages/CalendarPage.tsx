...
function statusBadgeClasses(key: string) {
  switch (key) {
    ...
    case "published":
      return "bg-[#3fa9f5]/10 text-[#3fa9f5] border-[#3fa9f5]/20";
    case "archived":
      return "bg-slate-400/10 text-slate-500 text-slate-500 dark:text-slate-500 border-slate-400/20";
    case "draft":
    default:
      return "bg-slate-300/20 text-slate-600 text-slate-700 border-slate-300/30";
  }
}
...
export function CalendarPage(props: CalendarPageProps) {
  ...
  const TabButton = ({ active, onClick, icon: Icon, label, count }: any) => (
    <button
      ...
      className={`relative flex items-center gap-2.5 px-6 py-5 text-sm font-bold transition-all border-b-2 z-10 ${active
        ? "text-blue-600 border-blue-600 bg-white bg-white"
        : "text-slate-500 text-slate-500 dark:text-slate-500 border-transparent hover:text-slate-700 text-slate-800 hover:bg-slate-50 dark:bg-slate-800/50/50"
        }`}
    >
      <Icon className={`w-4 h-4 ${active ? "text-blue-600" : "text-slate-400 dark:text-slate-500"}`} />
      {label}
      {count !== undefined && (
        <span className={`px-2 py-0.5 rounded-full text-[10px] ${active ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500 text-slate-500 dark:text-slate-500"}`}>
          {count}
        </span>
      )}
    </button>
  ...
  return (
    <main className="h-full flex flex-col overflow-hidden bg-gray-50 bg-gray-50/50 p-2.5 md:p-6 min-w-0 relative">
      ...
      <section className="w-full bg-white bg-white border border-slate-200 border-slate-200/60 rounded-2xl shadow-sm overflow-hidden flex flex-col h-full min-h-[850px] relative z-10">
        <header className="px-8 py-4 bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 border-b border-slate-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative group">
              <button className="p-3 bg-white bg-white/5 border border-white/10 rounded-xl text-white/70 hover:bg-white bg-white/10 transition">
                <MoreVertical className="w-4 h-4" />
              </button>
              <div className="absolute right-0 top-full mt-2 w-48 bg-white bg-white border border-slate-200 border-slate-200 rounded-xl shadow-xl opacity-0 group-hover:opacity-100 transition-all z-50 overflow-hidden pointer-events-none group-hover:pointer-events-auto">
                <button onClick={openCsvModal} className="w-full px-4 py-3 text-left text-xs font-bold text-slate-600 text-slate-700 hover:bg-slate-50 dark:bg-slate-800/50 flex items-center gap-2 border-b border-slate-50"><Download className="w-4 h-4" /> Export CSV</button>
                <button onClick={openCopyModal} className="w-full px-4 py-3 text-left text-xs font-bold text-slate-600 text-slate-700 hover:bg-slate-50 dark:bg-slate-800/50 flex items-center gap-2"><CopyIcon className="w-4 h-4" /> Copy to Sheets</button>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="px-4 py-3 min-w-[200px] md:min-w-[320px]">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800/50/50 border border-slate-200 border-slate-200 rounded-xl pl-10 pr-4 py-2 text-xs font-semibold outline-none transition-all focus:bg-white bg-white focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500/50"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-3 flex items-center gap-4 min-w-fit">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-2 px-4 py-2 bg-slate-100/50 hover:bg-slate-100 rounded-xl transition-all border border-slate-200 border-slate-200/50 group">
                    <Filter className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 group-hover:text-blue-500 transition-colors" />
                    <span className="text-[10px] font-black uppercase tracking-tight text-slate-600 text-slate-700">
                      Status: <span className="text-blue-600 ml-1">{calendarStatusFilter}</span>
                    </span>
                    <ChevronDown className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 group-hover:text-slate-600 text-slate-700 transition-transform duration-200" />
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56 p-2 bg-white bg-white/95 backdrop-blur-xl border-slate-200 border-slate-200/60 rounded-2xl shadow-2xl">
                  ...
                  {calendarStatusOptions.map((f: string) => (
                    <DropdownMenuItem
                      ...
                      className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold cursor-pointer transition-all mb-0.5 ${calendarStatusFilter === f
                        ...
                        : "text-slate-600 text-slate-700 hover:bg-slate-50 dark:bg-slate-800/50"
                        }`}
                    >
                      {f}
              ...
              <div className="flex items-center gap-1.5">
                <div className="flex bg-slate-100/50 p-1 rounded-xl gap-1">
                  <button title="List view" onClick={() => setDisplayType('list')} className={`p-1.5 rounded-lg transition-all ${displayType === 'list' ? 'bg-white bg-white text-blue-600 shadow-sm border border-blue-500/10' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 text-slate-700'}`}><ListIcon className="w-3.5 h-3.5" /></button>
                  <button title="Board view" onClick={() => setDisplayType('kanban')} className={`p-1.5 rounded-lg transition-all ${displayType === 'kanban' ? 'bg-white bg-white text-blue-600 shadow-sm border border-blue-500/10' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 text-slate-700'}`}><ColumnsIcon className="w-3.5 h-3.5" /></button>
                </div>
                ...
                <div className="relative">
                  <button
                    ...
                    className={`p-2 rounded-xl transition-all border ${showAddColumn
                      ...
                      : 'bg-white bg-white border-slate-200 border-slate-200 text-slate-400 dark:text-slate-500 hover:border-slate-300 hover:text-slate-600 text-slate-700'
                      }`}
                  >
                  ...
                  {showAddColumn && (
                    <div className="absolute right-0 top-full mt-2 z-50 w-64 bg-white bg-white border border-slate-200 border-slate-200 rounded-2xl shadow-xl p-4 animate-in zoom-in-95 fade-in duration-150">
                      ...
                      <input
                        ...
                        className="w-full border border-slate-200 border-slate-200 rounded-xl px-3 py-2 text-sm font-medium outline-none focus:ring-2 focus:ring-blue-400 mb-3"
                      />
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-2 bg-slate-100 text-slate-600 text-slate-700 text-xs font-black rounded-xl hover:bg-slate-200 transition-all"
                        >
                          Cancel
                        </button>
                      </div>
                ...
                <div className="relative">
                  ...
                  {showAutomations && (
                    <div className="absolute right-0 top-full mt-2 z-50 w-96 bg-white bg-white border border-slate-200 border-slate-200 rounded-[2rem] shadow-2xl p-8 animate-in zoom-in-95 fade-in duration-200 border-2 border-slate-100 border-slate-200">
                      ...
                      <p className="text-[11px] text-slate-500 text-slate-500 dark:text-slate-500 mb-8 font-medium leading-relaxed">
                ...
                <div className="relative">
                  ...
                  {showNotifications && (
                    <div className="absolute right-0 top-full mt-2 z-50 w-80 bg-white bg-white border border-slate-200 border-slate-200 rounded-[2rem] shadow-2xl p-8 animate-in zoom-in-95 fade-in duration-200 border-2 border-slate-100 border-slate-200">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600 mb-2">Notification Preferences</h4>
                      <p className="text-[11px] text-slate-500 text-slate-500 dark:text-slate-500 mb-6 font-medium leading-relaxed">
                      ...
                      <div className="mb-6 p-5 bg-blue-50/50 rounded-2xl border border-blue-100 flex items-center justify-between group hover:bg-blue-50 transition-all">
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-[10px] text-slate-500 text-slate-500 dark:text-slate-500 font-medium">Send summary email every minute</span>
                        </div>
                        <button
                          onClick={() => setEmailNotificationsEnabled(!emailNotificationsEnabled)}
                          className={`w-10 h-6 rounded-full transition-all relative ${emailNotificationsEnabled ? 'bg-blue-600' : 'bg-slate-200'}`}
                        >
                          <div className={`absolute top-1 w-4 h-4 rounded-full bg-white bg-white shadow-sm transition-all ${emailNotificationsEnabled ? 'right-1' : 'left-1'}`} />
                        </button>
                      </div>
                      ...
                      <div className="space-y-3 max-h-64 overflow-y-auto pr-2 mb-8 custom-scrollbar">
                        {kanbanColumns.map(col => (
                          <label key={col.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl cursor-pointer hover:bg-blue-50/50 transition-all border border-transparent hover:border-blue-100 group">
                            <div className="flex items-center gap-3">
                              <div className="w-2.5 h-2.5 rounded-full shadow-sm" style={{ backgroundColor: col.color }} />
                              <span className="text-xs font-bold text-slate-700 text-slate-800">{col.title}</span>
                            </div>
                            <input
                              type="checkbox"
                              checked={watchedColumns[col.id] || false}
                              onChange={() => toggleWatchColumn(col.id)}
                              className="w-5 h-5 rounded-lg border-slate-200 border-slate-200 text-blue-600 focus:ring-blue-500 transition-all cursor-pointer"
                            />
                          </label>
                        ))}
                      </div>
                ...
                <button title="Configure workflow" onClick={() => navigate(`/company/${companyId}/settings/workflow`)} className="p-2 bg-white bg-white border border-slate-200 border-slate-200 rounded-xl text-slate-400 dark:text-slate-500 hover:border-slate-300 hover:text-slate-600 text-slate-700 transition-all"><SettingsIcon className="w-4 h-4" /></button>
              </div>
            </div>
          </div>

          <div className="flex-1 overflow-hidden relative bg-white bg-white">
            {isLoadingCalendar ? <div className="p-10"><CalendarTableSkeleton /></div> : filteredCalendarRows.length === 0 ? (
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 bg-slate-50 dark:bg-slate-800/50/30">
                {currentPageRows.map(row => (
                  <div key={row.contentCalendarId} className="group bg-white bg-white border border-slate-200 border-slate-200 rounded-3xl overflow-hidden hover:shadow-xl transition-all cursor-pointer flex flex-col shadow-sm relative">
                    <div className="aspect-square bg-slate-100 relative" onClick={() => { setSelectedRow(row); setIsViewModalOpen(true); }}>
                      ...
                      <div className="absolute top-3 right-3 px-2 py-1 bg-white bg-white/90 backdrop-blur rounded-full text-[9px] font-black flex items-center gap-1.5 shadow-sm text-slate-500 text-slate-500 dark:text-slate-500 border border-slate-200 border-slate-200"><CheckCircle2 className="w-3 h-3 text-slate-400 dark:text-slate-500" /> Archived</div>
                    </div>
                    <div className="p-5 flex-1 flex flex-col" onClick={() => { setSelectedRow(row); setIsViewModalOpen(true); }}>
                      ...
                      <p className="text-xs font-semibold text-slate-700 text-slate-800 line-clamp-2 leading-relaxed mb-4">{row.finalCaption || row.captionOutput || 'No caption'}</p>
                    </div>
                    <div className="px-5 pb-5 mt-auto">
                      <button
                        ...
                        className="w-full py-2.5 bg-slate-100 hover:bg-white bg-white border border-slate-200 border-slate-200 hover:border-blue-500 rounded-xl text-[10px] font-black uppercase text-slate-600 text-slate-700 hover:text-blue-600 transition-all flex items-center justify-center gap-2 group/btn"
                      >
              ...
              <div className="overflow-auto h-full">
                <table className="w-full text-left border-collapse min-w-[1000px]">
                  <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 border-slate-200 z-10">
                    <tr>
                        ...
                        <tr key={row.contentCalendarId} className={`group hover:bg-slate-50 dark:bg-slate-800/50/80 transition-all ${isSelected ? "bg-blue-50/30" : ""}`}>
                          ...
                          <td className="px-4 py-5" onClick={() => { setSelectedRow(row); setIsViewModalOpen(true); }}>
                            <div className="flex gap-4">
                              <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 border-slate-200 flex items-center justify-center text-slate-400 dark:text-slate-500">{row.attachedDesign ? <Share2 className="w-5 h-5" /> : <FileText className="w-5 h-5" />}</div>
                          ...
                          <td className="px-4 py-5"><div className="flex flex-col gap-1">{row.primaryGoal && <div className="flex items-center gap-1.5"><Target className="w-3 h-3 text-blue-500" /><span className="text-[11px] font-bold text-slate-600 text-slate-700">{row.primaryGoal}</span></div>}</div></td>
                            ...
                            {row.collaborators && row.collaborators.length > 0 ? (
                              <div className="flex -space-x-1.5 overflow-hidden">
                                ...
                                {row.collaborators.length > 3 && (
                                  <div className="flex items-center justify-center h-6 w-6 rounded-full ring-2 ring-white bg-slate-100 text-[8px] font-bold text-slate-500 text-slate-500 dark:text-slate-500 shadow-sm">
                                    +{row.collaborators.length - 3}
                                  </div>
                                )}
                              </div>
          ...
          {!(viewMode === 'drafts' && displayType === 'kanban') && (
            <div className="px-6 py-4 border-t border-slate-100 border-slate-200 bg-white bg-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase">Size</span>
                <select value={pageSize} onChange={e => { setPage(1); setPageSize(e.target.value === "all" ? "all" : Number(e.target.value)); }} className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 border-slate-200 rounded-lg px-2 py-1 text-xs font-bold outline-none shadow-inner">
                  {[10, 25, 50, "all"].map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <span className="text-[11px] font-bold text-slate-500 text-slate-500 dark:text-slate-500">{currentPageRows.length} of {filteredCalendarRows.length}</span>
              </div>
              {pageSize !== "all" && (
                <div className="flex items-center gap-2">
                  <button disabled={page <= 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="p-1.5 rounded-lg border border-slate-200 border-slate-200 disabled:opacity-30"><ChevronLeft className="w-4 h-4" /></button>
                  <div className="px-3 py-1.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 border-slate-200 rounded-lg text-[11px] font-black"><span className="text-blue-600">{page}</span> / {Math.max(1, Math.ceil(filteredCalendarRows.length / (pageSize || 1)))}</div>
                  <button disabled={page >= Math.ceil(filteredCalendarRows.length / (pageSize || 1))} onClick={() => setPage(p => p + 1)} className="p-1.5 rounded-lg border border-slate-200 border-slate-200 disabled:opacity-30"><ChevronRight className="w-4 h-4" /></button>
                </div>
              )}
            </div>
          )}
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] animate-in fade-in zoom-in slide-in-from-bottom-5 duration-300">
          <div className="bg-slate-900 border border-white/10 shadow-2xl rounded-3xl p-3 flex items-center gap-6 text-white min-w-[400px]">
            ...
            <div className="h-10 w-[1px] bg-white bg-white/10" />
            <div className="flex gap-2 pr-2">
              <button onClick={handleBatchGenerate} disabled={isBatchGenerating || !userPermissions.canGenerate} className="px-5 py-2 bg-white bg-white text-slate-900 dark:text-white rounded-2xl text-[10px] font-black uppercase flex items-center gap-2">
      ...
      {pendingMove && (
        <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden p-8 text-center animate-in zoom-in-95 duration-300">
            ...
            <p className="text-sm text-slate-500 text-slate-500 dark:text-slate-500 font-medium mb-8">Moving to <span className="text-slate-900 dark:text-white font-bold">"{kanbanColumns.find(c => c.id === pendingMove.status)?.title}"</span> has automations. Run them?</p>
            <div className="space-y-2 mb-8 text-left">
              {pendingMove.rules.map((rule, idx) => (
                <div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 border-slate-200"><Wand2 className="w-4 h-4 text-blue-500" /><span className="text-[10px] font-black text-slate-700 text-slate-800 uppercase">{rule.action.replace('_', ' ')}</span></div>
              ))}
            </div>
            <label className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/50/50 rounded-2xl border border-slate-100 border-slate-200 mb-8 cursor-pointer">
              ...
              <span className="text-[10px] font-black uppercase text-slate-600 text-slate-700">Remember for this column</span>
            </label>
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => onConfirmMoveAction(false)} className="w-full py-4 bg-slate-100 text-slate-600 text-slate-700 rounded-2xl font-black uppercase text-sm">No, Just Move</button>
      ...
      {pendingApprovalMove && (
        <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden p-8 text-center animate-in zoom-in-95 duration-300">
            ...
            <p className="text-sm text-slate-500 text-slate-500 dark:text-slate-500 font-medium mb-8 leading-relaxed">
            ...
            <label className="flex items-center justify-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/50/50 rounded-2xl border border-slate-100 border-slate-200 mb-8 cursor-pointer group hover:bg-white bg-white hover:border-amber-200 transition-all">
              <input
                type="checkbox"
              ...
              />
              <span className="text-[11px] font-black uppercase text-slate-600 text-slate-700 tracking-wide">Remember my choice for this column</span>
            </label>
            <div className="flex flex-col gap-3">
              <button
                onClick={() => {
              ...
              <button
                ...
                className="w-full py-4 bg-slate-100 text-slate-600 text-slate-700 rounded-2xl font-black uppercase text-sm hover:bg-slate-200 transition-all"
              >
                Cancel Move
              </button>
            </div>
