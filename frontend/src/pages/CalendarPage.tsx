          ...
          <div className="bg-slate-900 border border-white/10 shadow-2xl rounded-3xl p-3 flex items-center gap-6 text-white min-w-[480px]">
            ...
            <div className="flex gap-2 pr-2">
              <button
                ...
                disabled={isBatchGenerating || !userPermissions.canGenerate}
                title={!userPermissions.canGenerate ? "You don't have permission to generate content" : ""}
                className={`px-5 py-2 bg-white text-slate-900 rounded-2xl text-[10px] font-black uppercase hover:bg-slate-100 transition-all flex items-center gap-2 ${!userPermissions.canGenerate ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
              ...
              <button
                ...
                disabled={isBatchDeleting || !userPermissions.canDelete}
                title={!userPermissions.canDelete ? "You don't have permission to delete content" : ""}
                className={`p-2 rounded-2xl transition-all ${isBatchDeleting ? 'bg-rose-500 text-white' : (userPermissions.canDelete ? 'hover:bg-rose-500 text-rose-500 hover:text-white' : 'text-slate-300 cursor-not-allowed')}`}
              >
